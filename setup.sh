...
#echo -e "\e[1;33m Do you want to build PX4 v1.10.1? (y) for simulation (n) if setting this up on on-board computer \e[0m"
#read var
#if [ "$var" != "y" ] && [ "$var" != "Y" ] ; then
...
grep -xF 'source $CATKIN_WS/devel/setup.bash' $HOMEDIR/setup-px4.sh || echo "source $CATKIN_WS/devel/setup.bash" >> $HOMEDIR/setup-px4.sh
